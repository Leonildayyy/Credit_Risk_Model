...
## The maximum value of pmts_pmtsoverdue_635A for each case_id.
# Whether there is any instance where pmts_dpdvalue_108P exceeds 31 days for each case_id.

# We will process in these examples only A-type and M-type columns, so we need to select them.
...
data.head(5)
print(data.select(pl.col("target").value_counts()))
...
##case_ids_train,case_ids_valid,case_ids_test
#先分training:0.6, 剩下0.4做test，再把test分两份，validation:0.2, testing:0.2
...
print(cols_pred) ##the features to build the model

#diving it into base,X and y
...
#apply function from_polars_to_pandas to case_ids_train, returning base_train, X_train, y_train
...
#lgb.Dataset: This creates a dataset object specifically for LightGBM, which is optimized for faster training.
...
params = {
    ...
    "n_estimators": 1000,
    "verbose": -1,
    "scale_pos_weight": 1478665 / 47994, # adjust for class imbalance
}
...
#This loop iterates over three pairs of datasets: base_train with X_train, base_valid with X_valid, and base_test with X_test.
for base, X in [(base_train, X_train), (base_valid, X_valid), (base_test, X_test)]:
    y_pred = gbm.predict(X, num_iteration=gbm.best_iteration)
    base["score"] = y_pred  #The predicted probabilities (y_pred) are then added as a new column called score in base_train

print(f'The AUC score on the train set is: {roc_auc_score(base_train["target"], base_train["score"])}')
print(f'The AUC score on the valid set is: {roc_auc_score(base_valid["target"], base_valid["score"])}')
print(f'The AUC score on the test set is: {roc_auc_score(base_test["target"], base_test["score"])}')

def evaluate_cutoffs(y_true, y_pred_proba, cutoffs=[0.3, 0.5, 0.7]):
    for c in cutoffs:
...
evaluate_cutoffs(base_test["target"], base_test["score"])